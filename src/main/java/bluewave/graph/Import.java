...
public class Import {
  ...
  public static void importCSV(File csvFile, String vertex, Integer[] keys, Neo4J database)
      throws Exception {
    ...
    if (keys != null) {
      ...
      try {
        ...
        try {
          session.run(
              "CREATE CONSTRAINT ON (n:" + vertex + ") ASSERT n." + uniqueColName + " IS UNIQUE");
        } catch (Exception e) {
        }
   ...
   * @param target Used to identify a specific path in the json object to start parsing (e.g.
   *     "/results")
   * @param database Connection info to the database
   */
  public static void importJSON(File jsonFile, String vertex, String target, Neo4J database)
      throws Exception {
    ...
    ThreadPool pool =
        new ThreadPool(12, 1000) {
          ...
          private void parseJson(
              ...
              String target,
              JsonToken currToken,
              LinkedHashMap<String, Boolean> currPath,
              Long jobID)
              throws Exception {
            ...
            try {
              while (!endOfFile && !endOfNode) {
                ...
                // BEGIN_ARRAY, END_ARRAY, BEGIN_OBJECT, END_OBJECT, NAME, STRING, NUMBER, BOOLEAN,
                // NULL, END_DOCUMENT;
                switch (nextToken) {
                  case BEGIN_ARRAY:
                    jr.beginArray();
  ...
  // **************************************************************************
  /**
   * Returns a node label for a given path. Uses an underscore character to separate node names in
   * the path. If a node in the path corresponds to a JSON array, the node name is updated to it's
   * singular form.
   */
  ...
  // ** addRow
  // **************************************************************************
  private static Result addRow(
      final Transaction tx, final String query, Map<String, Object> params) {
