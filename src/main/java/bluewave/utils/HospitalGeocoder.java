...
// ******************************************************************************
/**
 * Used to generate lat/lon coordinates for Hospitals found in the HHS report "COVID-19 Reported
 * Patient Impact and Hospital Capacity by Facility"
 * https://healthdata.gov/dataset/covid-19-reported-patient-impact-and-hospital-capacity-facility
 *
 * <p>The geocoder uses 3 different sources including DHS/HSIS, USGS, and Google. Google is a paid
 * service and should be used as a last resort to avoid fees.
 *
 * <p>****************************************************************************
 */
public class HospitalGeocoder {
    ...
    while ((row = br.readLine()) != null) {
      ...
      if (numMatches == 0) {
        ...
        for (JSONObject hospital : arr) {
          ...
          try {
            double cityScore =
                FuzzySearch.tokenSortPartialRatio(city, hospital.get("city").toString());
            if (cityScore > 90) {
              String hospitalName = hospital.get("name").toString();
  ...
  public void geocodeUSGS(String usgsFile) throws Exception {
    ...
    while ((row = br.readLine()) != null) {
      ...
      if (numMatches == 0
          && (feature.equals("Hospital")
              || feature.equals("Building")
              || feature.equals("School"))) {
        String a = name.toLowerCase(); // .replace("hospital", "");
        for (JSONObject hospital : arr) {
          String hospitalID = hospital.get("id").toString();
          if (coords.containsKey(hospitalID)) continue;
    ...
    while (it.hasNext()) {
      ...
      if (!coords.containsKey(id)) {
        ...
        javaxt.http.Response response =
            new javaxt.http.Request(url.toString().replace(" ", "%20")).getResponse();
        ...
        for (int i = 0; i < results.length(); i++) {
          ...
          updateHospital(
              coords.get("lat").toString(), coords.get("lon").toString(), geocoder, hospital);
          break;
        }
      }
    }
  ...
  public String toString() {
    ...
    Set<String> keys =
        hospitals
            .get(hospitals.keySet().iterator().next())
            .keySet(); // assumes first entry has all the keys!
