...
public class GraphService extends WebService {
  ...
  // ** getRelationships
  // **************************************************************************
  public ServiceResponse getRelationships(ServiceRequest request, Database database)
      throws ServletException, IOException {
    Session session = null;
    try {
      ...
      if (request.hasParameter("id")) {
        ...
        String query =
            bluewave.queries.Index.getQuery("Nodes_And_Edges_By_Selected_Node", "cypher");
    ...
    synchronized (cache) {
      ...
      if (obj != null) {
        ...
        for (Vertex v : graph.getVertices()) {
          Geometry geom = geometryFactory.createPoint(new Coordinate(v.getPos().x, v.getPos().y));
