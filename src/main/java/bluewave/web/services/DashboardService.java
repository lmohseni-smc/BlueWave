...
public class DashboardService extends WebService {
  ...
  // ** Constructor
  // **************************************************************************
  public DashboardService(bluewave.web.WebServices ws, javaxt.io.Directory web, Database database)
      throws Exception {
    this.ws = ws;
  ...
  // ** getRecordset
  // **************************************************************************
  /** Used to apply filters before fetching dashboards or executing any CRUD operations */
  protected Recordset getRecordset(
      ServiceRequest serviceRequest, String op, Class c, String sql, Connection conn)
      throws Exception {
  ...
  public ServiceResponse saveDashboard(ServiceRequest request, Database database)
    ...
    try {
      ...
      // Fire event
      if (isNew) onCreate(dashboard, request);
      else onUpdate(dashboard, request);
  ...
  private boolean isAuthorized(
      bluewave.app.User user, Dashboard dashboard, Database database, boolean readOnly)
      throws Exception {
    ...
    try {
      ...
      if (readOnly) {
        ...
        sql =
            "select read_only from APPLICATION.DASHBOARD_USER where "
                + "dashboard_id="
                + dashboard.getID()
                + " and user_id="
                + user.getID();
