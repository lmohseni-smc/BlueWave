...
// ******************************************************************************
/**
 * Used to find, create, edit, and delete supply chain data (company, facility, product, etc).
 * Leverages registrationlisting data from OpenFDA
 *
 * <p>****************************************************************************
 */
public class SupplyChainService extends WebService {
  ...
  /**
   * Returns a list of companies that start with a given name. Searches both "company" nodes and
   * "registrationlisting" nodes
   */
  public ServiceResponse getCompanies(ServiceRequest request, Database database)
    ...
    try {
      ...
      try {
        String query =
            "MATCH (c:company)\n"
                + "WHERE toLower(c.name) STARTS WITH toLower(\""
                + name
                + "\")\n"
      ...
      try {
        String query =
            "MATCH (o:registrationlisting_registration_owner_operator)-[:has]->(a:registrationlisting_registration_owner_operator_contact_address)\n"
                + "WHERE toLower(o.firm_name) STARTS WITH toLower(\""
                + name
                + "\")\n"
  ...
  /** Returns company info from the owner_operator and address nodes in the registrationlisting */
  ...
  public ServiceResponse saveCompany(ServiceRequest request, Database database)
    ...
    try {
      ...
      if (companyID == null) {
        ...
        if (sourceID != null) {
          session.run(
              "MATCH (c),(o:registrationlisting_registration_owner_operator {owner_operator_number:\""
                  + sourceID
                  + "\"}) "
                  + "WHERE id(c)="
                  + companyID
          ...
          // MATCH (n:company) WHERE id(n)=12438225 MATCH (n)-[r]-(p) RETURN id(n), labels(n),
          // type(r), labels(p)
        }
  ...
  /**
   * Returns a list of facilities associated with a given company. Searches both "facility" nodes
   * and "registrationlisting" nodes
   */
  ...
  private JSONArray getFacilities(Long ownerOperatorID, Session session) throws Exception {
    ...
    if (ownerOperatorID != null) {
      ...
      String query =
          "MATCH (registration:registrationlisting_registration)-[:has]->"
              + "(n:registrationlisting_registration_owner_operator {owner_operator_number: '"
              + ownerOperatorID
              + "'})\n"
  ...
  public ServiceResponse saveFacility(ServiceRequest request, Database database)
    ...
    if (state != null) {
      if (state.length() != 2)
        throw new ServletException(400, "Invalid state. 2 char state code is required");
    }

    if (country != null) {
      if (country.length() != 2)
        throw new ServletException(400, "Invalid country. 2 char country code is required");
    }
    ...
    try {
      ...
      if (facilityID == null) {
        ...
        if (sourceID != null) {
          session.run(
              "MATCH (f),(n:registrationlisting_registration {fei_number:\""
                  + sourceID
                  + "\"}) "
                  + "WHERE id(f)="
                  + facilityID
                  + " MERGE(f)-[:source]->(n)");
          // To confirm link, run something like this:
          // MATCH (n:facility) WHERE id(n)=12438225 MATCH (n)-[r]-(p) RETURN id(n), labels(n),
          // type(r), labels(p)
        }
  ...
  /**
   * Returns a unique list of product codes associated with a given facility. Searches both
   * "product" nodes and "registrationlisting" nodes.
   */
  public ServiceResponse getProducts(ServiceRequest request, Database database)
    ...
    if (facilityID != null) {
      ...
      if (fei != null) {
        ...
        String name = request.getParameter("name").toString();
        if (name == null)
          return new ServiceResponse(400, "Product name, facilityID, or fei is required");
        ...
        try {
          ...
          String query =
              "MATCH (p:registrationlisting_product)-[:has]->(n:registrationlisting_product_openfda)\n"
                  + "WHERE toLower(n.device_name) CONTAINS toLower('"
                  + name
                  + "')\n"
   ...
   *
   * @param fei Facility Establishment Number. FDA-assigned unique id for a facility
   */
  private JSONArray getProducts(Long fei, Session session) throws Exception {
    ...
    if (fei != null) {
      ...
      String query =
          "MATCH (:registrationlisting_registration {fei_number:\""
              + fei
              + "\"})<-[:has]-(r:registrationlisting)-[:has]->(p:registrationlisting_product)-[:has]->(o:registrationlisting_product_openfda)\n"
              + "RETURN "
    ...
    try {
      ...
      if (productID == null) {
        ...
        if (rs.hasNext()) {
          ...
          if (inventory == null || !inventory.equals(productInventory))
            updates.put("inventory", productInventory);

          String capacity = product.get("capacity").toString();
          if (capacity == null || !capacity.equals(productCapacity))
            updates.put("capacity", productCapacity);

          String leadTime = product.get("leadTime").toString();
          if (leadTime == null || !leadTime.equals(productLeadTime))
            updates.put("leadTime", productLeadTime);
        }
  ...
  /**
   * Returns a string for the given value. Returns null if the string is empty or equals "NULL"
   * (case insensitive)
   */
