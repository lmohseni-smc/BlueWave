...
public class QueryService extends WebService {
  ...
  // ** createWebSocket
  // **************************************************************************
  public void createWebSocket(HttpServletRequest request, HttpServletResponse response)
      throws IOException {
  ...
  // ** getJob
  // **************************************************************************
  /** Used to return the status or results for a given jobID. Example: [GET] sql/job/{jobID} */
  ...
  // ** getJob
  // **************************************************************************
  /** Returns a job for a given jobID and user. Checks both the pending and completed job queues. */
  ...
  // **************************************************************************
  /**
   * Used to generate a ServiceResponse for a given job. If a job has failed or is complete, returns
   * the output of the job. If the job is pending or running, simply returns the job status.
   */
  ...
  // **************************************************************************
  /**
   * Removes a job from the queue and deletes any output files that might have been created with the
   * job.
   */
  ...
  // **************************************************************************
  /**
   * Used to extract a parameter either from the URL query string or the json in the request
   * payload.
   */
        ...
        for (int i = 0; i < fields.size(); i++) {
          ...
          if (value == null) {
            ...
            if (value instanceof String) {
              ...
              value =
                  "\"" + map
                      + "\""; // <- this needs to be improved, values in the map can contain quotes
            } else {
  ...
  public class QueryJob {
    ...
    public javaxt.io.File[] getOutputs() {
      ...
      return new javaxt.io.File[] {
        new javaxt.io.File(path + format), new javaxt.io.File(path + "cypher")
      };
    }
    ...
    public String getContentType() {
      if (format.equals("tsv")) {
        return "text/plain";
      } else if (format.equals("csv")) return "text/csv";
      else {
