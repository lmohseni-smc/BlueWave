...
public class WebApp extends HttpServlet {
  ...
  public WebApp() throws Exception {
    ...
    javaxt.io.File configFile = new javaxt.io.File(jar.getFile().getParentFile(), "config.json");
    ...
    if (service.equals("login")) {
      if (credentials == null) {
        if (ntlm) {
          ...
          response.setHeader(
              "WWW-Authenticate",
              "Basic realm=\"Access Denied\""); // <--Prompt the user for thier credentials
      ...
      if (ntlm) {
        ...
        response.setStatus(401, "Access Denied");
        Boolean prompt =
            new javaxt.utils.Value(request.getParameter("prompt"))
                .toBoolean(); // <--Hack for Firefox
  ...
  // **************************************************************************
  /**
   * Used to convert http request headers and metadata into a string. Updates the "Authorization"
   * header to hide passwords.
   */
