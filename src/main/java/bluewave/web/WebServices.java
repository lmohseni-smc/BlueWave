...
public class WebServices extends WebService {
   ...
   *
   * @param service The first "directory" found in the path, after the servlet context.
   */
  ...
  // **************************************************************************
  /**
   * Maps a ServiceRequest to a WebService. Returns a ServiceResponse object to send back to the
   * client.
   */
  ...
  protected Recordset getRecordset(
    ...
    if (c.equals(bluewave.app.User.class)) {
      if (op.equals("list")) {
        ...
        // Remove password field for most requests - except admins (admins need to have password to
        // save users)
  ...
  public void notify(String action, Model model, bluewave.app.User user) {
    ...
    synchronized (listeners) {
      ...
      while (it.hasNext()) {
        ...
        ws.send(
            action + "," + model.getClass().getSimpleName() + "," + model.getID() + "," + userID);
      }
    }
  }
}
